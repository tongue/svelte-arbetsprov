...
<style>
	...
	:global(:root) {
		...
		--color-neutral: var(--color-brand-yellow);
	...
	:global(#svelte) {
		...
		display: grid;
		min-height: 100vh;
		grid-template-rows: minmax(var(--spacing-gap), auto) min-content var(--spacing-gap) min-content var(--spacing-gap) auto minmax(
				var(--spacing-gap),
				auto
			) min-content var(--spacing-gap);
